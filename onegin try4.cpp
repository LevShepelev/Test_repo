...
int filltext (FILE* fin, struct str* text, struct str* copyt) //Функция считывает текст из файла и заполняет его в массив структур
    {
    const int mem = 250;
    int nrealoc = 1;
    int numb = 0;
...
int cmp2 (struct str text1, struct str text2)  // Компаратор для рифмованного текста
    {
    int i = 0;
    int j = 0;
...
int cmp1 (struct str text1, struct str text2)  // обычный компаратор для строк
    {
    int i = 0;
    int j = 0;
    while ((i < text1.l) && (j < text2.l))
...
void bubbleSort(struct str* text, int nstr, int (*cmp)(struct str, struct str) )   // Функция сортирует массив из строчек
    {
    struct str temp;
    int k = 0, m = 0;
    for (k = 0; k < nstr; k++)

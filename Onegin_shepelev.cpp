...
void exchange(char* text, int x, int y, int nstr)  // Функция обменивает строчки
    {
    char chng;
...
int strcmp(char* s1, int y, int x)    // Функция сравнивает строчки
    {
    int i = 0;
    for (i = 0; ((s1[256*y + i] != '\n') && (s1[256*y + i] != '\0')); i++)
        {
...
void bubbleSort(char* text, int nstr)   // Функция сортирует массив из строчек
    {
        int k, m;
        for (k = 0; k < nstr; k++)
        {
...
int main()
    ...
    FILE* fout = fopen("second.txt", "w"); //называть fin файл для вывода как то странно
    ...
    fclose(fout);
    printf("Executed\n"); //пишем что программа завершилась
    return 0;
    }
